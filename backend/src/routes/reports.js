...
// they can't drift out of sync with the source data. Piece counts (never
// dollar totals — subtotal is already bundle-price x bundle-count, which
// is correct as-is) are multiplied by pieces_per_bundle since order_items
// quantity is a bundle count, not a piece count.
router.get('/sales', async (req, res) => {
  ...
  const summary = await pool.query(
    `SELECT
       ...
       COALESCE(SUM(oi.quantity * COALESCE(p.pieces_per_bundle, 1)), 0) AS total_bibingka_sold,
  ...
  const byProduct = await pool.query(
    `SELECT p.name,
            SUM(oi.quantity * COALESCE(p.pieces_per_bundle, 1)) AS quantity,
...
router.get('/best-sellers', async (req, res) => {
  ...
  const { rows } = await pool.query(
    `SELECT p.name, SUM(oi.quantity * COALESCE(p.pieces_per_bundle, 1)) AS total_sold
     FROM order_items oi
     JOIN orders o ON o.id = oi.order_id
     JOIN products p ON p.id = oi.product_id
     WHERE o.status != 'cancelled'
